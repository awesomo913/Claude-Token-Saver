...
- [`_initial_build`](utilities/_initial_build.py) (from `NeoAutocoder/core_loop.py:139`) -- Initial codebase generation.
- [`_expansion_mode`](utilities/_expansion_mode.py) (from `NeoAutocoder/core_loop.py:161`) -- Generate companion modules when stagnant.
- [`_save_iteration`](utilities/_save_iteration.py) (from `NeoAutocoder/core_loop.py:174`) -- Organized auto-save per Fix 8.
- [`_extract_project_slug`](utilities/_extract_project_slug.py) (from `NeoAutocoder/core_loop.py:198`) -- Extract project name from code or task for organized storage.
- [`get_next_provider`](utilities/get_next_provider.py) (from `NeoAutocoder/provider_chain.py:97`) -- Get next available provider, respecting cooldowns and rotation.
- [`generate`](utilities/NeoAutocoder__generate.py) (from `NeoAutocoder/provider_chain.py:129`) -- Main entrypoint - try providers in order with fallbacks.
- [`get_current_focus`](utilities/get_current_focus.py) (from `NeoAutocoder/provider_chain.py:176`) -- Cycle through curated focuses.
- [`save_config`](utilities/save_config.py) (from `NeoAutocoder/provider_chain.py:180`) -- Persist provider stats.
- [`update_hash`](utilities/update_hash.py) (from `NeoAutocoder/session.py:29`) -- Update stagnation detection hash.
- [`is_stagnant`](utilities/is_stagnant.py) (from `NeoAutocoder/session.py:38`) -- Detect if code stopped evolving.
- [`save_state`](utilities/save_state.py) (from `NeoAutocoder/session.py:45`) -- Persist session state.
...
- [`_score_project`](utilities/_score_project.py) (from `claude_backend/overlay.py:66`) -- Score how likely proj matches the window the user was working in.
- [`open_overlay`](utilities/open_overlay.py) (from `claude_backend/overlay.py:751`) -- Construct + return an overlay attached to `parent`.
- [`_restore_position`](utilities/_restore_position.py) (from `claude_backend/overlay.py:174`) -- Get saved position or default to top-right of primary monitor.
- [`_save_position`](utilities/_save_position.py) (from `claude_backend/overlay.py:186`) -- Persist current position to prefs.
- [`_set_alpha`](utilities/_set_alpha.py) (from `claude_backend/overlay.py:252`) -- Set window translucency; clamped + cached so we don't pound
- [`_cursor_near`](utilities/_cursor_near.py) (from `claude_backend/overlay.py:264`) -- True if the global mouse cursor is within FADE_PROXIMITY_PX
- [`_poll_proximity`](utilities/_poll_proximity.py) (from `claude_backend/overlay.py:282`) -- Tick: restore alpha when cursor near, fade out after idle.
- [`_post_improve`](utilities/_post_improve.py) (from `claude_backend/overlay.py:672`) -- POST to /improve. The GUI picks up the result via pending file.
- [`_show_error_toast`](utilities/_show_error_toast.py) (from `claude_backend/overlay.py:717`) -- Tiny self-dismissing CTkToplevel near the overlay reporting failure.
...
- [`_log_session`](utilities/_log_session.py) (from `claude_backend/session_launcher.py:46`) -- Append one timestamped line to today's session log. Never raises.
- [`_http_is_alive`](utilities/_http_is_alive.py) (from `claude_backend/session_launcher.py:63`) -- Return True if the Token Saver HTTP server responds on localhost.
- [`_clear_stale_lock`](utilities/_clear_stale_lock.py) (from `claude_backend/session_launcher.py:74`) -- Remove pidfile if the recorded PID is dead or reused by another process.
- [`_verify_http_in_background`](utilities/_verify_http_in_background.py) (from `claude_backend/session_launcher.py:113`) -- Poll HTTP server in a background thread and log when it comes up (or times out).
- [`_find_exe`](utilities/_find_exe.py) (from `claude_backend/session_launcher.py:132`) -- Locate ClaudeTokenSaver.exe on disk. Prefer Desktop deploy.
- [`_spawn_detached`](utilities/_spawn_detached.py) (from `claude_backend/session_launcher.py:140`) -- Spawn a detached subprocess so the hook can return immediately.
...
- [`Session`](classes/NeoAutocoder__Session.py) (from `NeoAutocoder/session.py:9`) -- Exact match to original Session dataclass.
- [`ProviderEntry`](classes/ProviderEntry.py) (from `NeoAutocoder/session.py:62`) -- Exact match to original ProviderEntry.
...
- [`__init__`](patterns/NeoAutocoder____init__.py) (from `NeoAutocoder/core_loop.py:31`)
- [`__init__`](patterns/NeoAutocoder____init___2.py) (from `NeoAutocoder/provider_chain.py:29`)
...
- [`__init__`](patterns/claude_backend____init___4.py) (from `claude_backend/overlay.py:133`)
- [`_build`](patterns/_build.py) (from `claude_backend/overlay.py:196`)
